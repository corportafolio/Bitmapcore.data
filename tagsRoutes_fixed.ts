...
router.get('/tags', (req: Request, res: Response) => {
  try {
    ...
    let tagList: any[] = [];
    if (tableExists(db, 'tag_tables')) {
      tagList = db.prepare('SELECT * FROM tag_tables ORDER BY lastUpdated DESC').all();
    }
    ...
    // txS millonarias - case-sensitive 'millonaria'
    try {
    ...
    // TXs MULTIMILLONARIAS - case-insensitive 'multimillonaria'
    try {
...
router.get('/tags/:tagName/preview', (req: Request, res: Response) => {
  try {
    const tagName = req.params.tagName;
    const db = getBlocksDb();
    
    if (!tableExists(db, 'tagged_blocks') || !tableExists(db, 'tag_tables')) {
    ...
    if (tableExists(getBlocksDb(), 'blocks')) {
      block = getBlocksDb().prepare('SELECT * FROM blocks WHERE bloque = ?').get(tagged.bloque);
    }
